...
CONSERVATIVE_ATTR(print_stats, 0666, conservative_stats_show, conservative_stats_store);
...
CONSERVATIVE_ATTR(polling_interval, 0666, conservative_polling_interval_show, conservative_polling_interval_store);

static struct attribute *conservative_attrs[] = {
    &conservative_attr_print_stats.attr,
    &conservative_attr_polling_interval.attr,
    NULL
};

static ssize_t conservative_sysfs_show(struct kobject *kobj,
				   struct attribute *attr, char *buf)
{
	struct kgsl_device *device = to_device(kobj);
	struct kgsl_conservative_attribute *pattr = to_pwrscale_attr(attr);
	ssize_t ret;
...
static ssize_t conservative_sysfs_store(struct kobject *kobj,
				    struct attribute *attr,
...
static void conservative_sysfs_release(struct kobject *kobj)
{
}

static const struct sysfs_ops conservative_sysfs_ops = {
	.show = conservative_sysfs_show,
	.store = conservative_sysfs_store
};

static struct kobj_type ktype_conservative = {
	.sysfs_ops = &conservative_sysfs_ops,
	.default_attrs = conservative_attrs,
	.release = conservative_sysfs_release
};
...
static int conservative_init(struct kgsl_device *device, struct kgsl_pwrscale *pwrscale)
{
    ...
    ret = kobject_init_and_add(&device->pwrscale_conservative_kobj, &ktype_conservative,
                &device->pwrscale_kobj, "conservative");
...
{
    kobject_del(&device->pwrscale_conservative_kobj);
    kobject_put(&device->pwrscale_conservative_kobj);
}
